...
// Делаем прогресс глобальным, чтобы Tank.js мог его видеть
...
function gameLoop(timestamp) {
    ...
    for (let i = mines.length - 1; i >= 0; i--) {
        ...
        for (let e of enemies) {
            if (e.hp <= 0) continue;
            let d = Math.sqrt(Math.pow(m.x - e.x, 2) + Math.pow(m.y - e.y, 2));
    ...
    // ОТРИСОВКА МИН-ПАУКОВ
    for (let m of mines) {
        ctx.shadowBlur = 10; ctx.shadowColor = '#ff0000'; ctx.fillStyle = '#333';
        ctx.beginPath(); ctx.arc(m.x, m.y, m.radius, 0, Math.PI * 2); ctx.fill();
        ctx.fillStyle = Math.floor(timestamp/200)%2 === 0 ? '#ff0000' : '#440000';
        ctx.beginPath(); ctx.arc(m.x, m.y, 3, 0, Math.PI * 2); ctx.fill(); ctx.shadowBlur = 0;
    }
