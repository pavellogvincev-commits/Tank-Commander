...
playerImages.hulls["hunter"].src = 'assets/hull.png' + noCache; 
...
function spawnText(x, y, text, color) { 
    floatingTexts.push({ x, y, text, color, life: 1.5, maxLife: 1.5, vy: -30 }); 
}
...
function spawnSparks(x, y, nx, ny) { 
    ...
    for (let i = 0; i < count; i++) { 
        ...
        sparks.push({ 
            x, y, 
            vx: Math.cos(base + spread) * speed, 
            vy: Math.sin(base + spread) * speed, 
...
function startLevel(levelNum) {
    ...
    let bHull = GameData.hulls[hullId]; 
    let sHull = PlayerProgress.partStats[hullId];
    ...
    let bTurr = GameData.turrets[turretId]; 
    let sTurr = PlayerProgress.partStats[turretId];
...
function spawnEnemyOnArena() {
    ...
    do {
        ...
        spawnY = 100 + Math.random() * (canvas.height - 200);
        ...
        if (validSpawn) { 
            for (let e of enemies) { 
                ...
                if (distToEnemy < 100) { validSpawn = false; break; } 
            } 
        }
        attempts++;
...
function gameLoop(timestamp) {
    ...
    if (playerTank.hp > 0) {
        ...
        for (let i = 0; i < pShots; i++) { 
            ...
            bullets.push(new Bullet(startX, startY, playerTank.turretAngle, playerTank, playerTank.penetration, playerTank.bulletRadius, playerTank.bulletColor, playerTank.bulletSpeed)); 
    ...
    if (playerTank && playerTank.hp > 0) {
        for (let i = drops.length - 1; i >= 0; i--) {
            ...
            if (dist < playerTank.radius + d.radius) {
                if (d.type === 'hull') { 
                    PlayerProgress.inventory.hullUpgrades++; 
                    spawnText(d.x, d.y, "+1 Корпус", '#00ccff'); 
                } else { 
                    PlayerProgress.inventory.turretUpgrades++; 
                    spawnText(d.x, d.y, "+1 Башня", '#ff3333'); 
                }
    ...
    for (let d of drops) {
        ctx.shadowBlur = 15; 
        ctx.shadowColor = d.type === 'hull' ? '#00ccff' : '#ff3333';
    ...
    if (playerTank && playerTank.hp > 0) {
        ...
        if (playerTank.hullName === "Леопард") {
            if (playerTank.droneState === 'ready') {
                ...
                ctx.fillStyle = '#00ffcc';
                ctx.beginPath(); 
                ctx.arc(playerTank.droneX, playerTank.droneY, 4, 0, Math.PI * 2); 
                ctx.fill(); 
                ctx.shadowBlur = 0;
