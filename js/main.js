...
const GameData = {
    ...
    turrets: { 
        "scourge": { name: "Плеть", fireRate: 1.0, penetration: 80, burstCount: 1, burstDelay: 0 } 
    },
    enemyHulls: { 
    ...
    enemyTurrets: { 
        "basic": { name: "Враг-Пушка", fireRate: 2.0, penetration: 60, burstCount: 1, burstDelay: 0 },
        "scout": { name: "Скаут-Автопушка", fireRate: 2.0, penetration: 35, burstCount: 3, burstDelay: 0.15 } 
    }
};

let currentAssembly = { hullId: "hunter", turretId: "scourge" };
let PlayerProgress = { unlockedLevel: 2 }; 

const LevelsConfig = { 
    1: { pool: ["basic", "basic", "basic", "basic"], bonuses: 2 },
    2: { pool: ["basic", "basic", "basic", "scout", "scout"], bonuses: 3 }
};
...
function updateHangarUI() {
    let hData = GameData.hulls[currentAssembly.hullId]; let tData = GameData.turrets[currentAssembly.turretId];
    document.getElementById('stat-hp').innerText = hData.hp; document.getElementById('stat-armor').innerText = `${hData.armor.front} / ${hData.armor.side} / ${hData.armor.rear}`;
    document.getElementById('stat-speed').innerText = hData.speed; document.getElementById('stat-penetration').innerText = tData.penetration;
}
updateHangarUI();
...
function generateLevelsGrid() {
    ...
    for (let i = 1; i <= 100; i++) {
        ...
        btn.innerHTML = `<div>${i}</div>` + (i === 1 ? `<div class="bonuses">★ ★</div>` : '');
...
function spawnSparks(x, y, nx, ny) {
    ...
    for (let i = 0; i < count; i++) {
        let spread = (Math.random() - 0.5) * Math.PI; let speed = 100 + Math.random() * 200; 
        sparks.push({ x, y, vx: Math.cos(base+spread)*speed, vy: Math.sin(base+spread)*speed, life: 0.2+Math.random()*0.2, maxLife: 0.4, size: 2+Math.random()*3, color: '255, 200, 0' });
    }
}
function spawnExplosion(x, y) {
    playSound(explodeSound); let colors = ['255, 50, 0', '255, 150, 0', '100, 100, 100', '40, 40, 40']; 
    for (let i = 0; i < 100; i++) { 
        let a = Math.random() * Math.PI * 2; let s = 50 + Math.random() * 350;
...
function startLevel(levelNum) {
    ...
    enemiesToSpawn = currentEnemyPool.length;
    enemySpawnTimer = 0; levelFinished = false;
...
function spawnEnemyOnArena() {
    ...
    do {
        ...
        if (dist > safeDist && !arena.checkCollision(spawnX, spawnY, 25)) break;
        attempts++;
    } while (attempts < 50);
